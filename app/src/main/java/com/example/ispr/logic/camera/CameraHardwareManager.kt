...
import com.example.ispr.logic.camera.processing.ProcessingFrameProcessor
...
class CameraHardwareManager(private val context: Context) {
	...
	fun setPreviewSurface(surface: android.view.Surface?) =
		cameraStreamManager.setPreviewSurface(surface)
	...
	private fun extractCameraInfo(chars: CameraCharacteristics): CameraHardwareInfo {
		...
		val supportedResolutions =
			streamMap?.getOutputSizes(ImageFormat.YUV_420_888)?.toList() ?: emptyList()
		val fpsRanges =
		...
		val resolutionMaxFps = supportedResolutions.associateWith { size ->
			val minFrameDuration =
				streamMap?.getOutputMinFrameDuration(ImageFormat.YUV_420_888, size) ?: 0L
		...
		return CameraHardwareInfo(
			...
			facing = "Front",
			sensorOrientation = chars.get(CameraCharacteristics.SENSOR_ORIENTATION)?.toString()
				?: "Unknown",
